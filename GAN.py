...
Original file is located at
    https://colab.research.google.com/drive/13zioWTaw474RZMdejUfuFl4weFvdMXhG
"""
...
def show_result(G_net, z_, num_epoch, show=False, save=False, path='result.png'):
    ...
    image_test = G_net(z_)
    fig_grid_size = 5
    fig, axis = plt.subplots(fig_grid_size, fig_grid_size, figsize=(5, 5))
    for x, y in itertools.product(range(fig_grid_size), range(fig_grid_size)):
        axis[x, y].get_xaxis().set_visible(False)
        axis[x, y].get_yaxis().set_visible(False)
    
    for z in range(fig_grid_size*fig_grid_size):
        x = z // 5
        y = z % 5
        axis[x, y].cla()
        axis[x, y].imshow(image_test[z, :].cpu().data.view(28, 28).numpy(), cmap='gray')

    fig.text(0.5, 0.005, 'Epoch %s'%num_epoch, horizontalalignment='center')
